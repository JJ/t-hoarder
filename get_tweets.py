...
def get_user(api,user,flag_id_user,f_log):
  error = False  
  profile_text = '%s\tNone\n' % (user)
  print 'Getting user profile ',user,flag_id_user
...
#  print 'file name', file_users
  ...
  try:
    ...
    print 'Users group file does not exist'
    exit (1)   
  ...
  # acciones
  if flag_profile:
    f_out=  codecs.open(prefix+'_profiles.txt', 'a',encoding='utf-8', errors='ignore') 
    ...
    f_out.close()
  if flag_tweets:
    f_out=  codecs.open(prefix+'_tweets.txt','a',encoding='utf-8')
